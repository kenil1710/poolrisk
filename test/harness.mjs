/**
 * Shared helpers for the PoolRisk integration scripts.
 *
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      const res = await fetch(chain.rpcUrls.default.http[0], {
        ...
        // The amount is written as a RAW INTEGER LITERAL. `Number(wei)` for
        // anything above 2^53 serialises as `2e+21`, which the endpoint answers
        // with "amount must be a positive integer" - and because funding is
        // non-fatal, that failure was silent: every account stayed at zero.
        body: `{"jsonrpc":"2.0","id":1,"method":"sim_fundAccount","params":["${address}",${BigInt(wei).toString()}]}`,
        signal: AbortSignal.timeout(20_000),
      });
      const json = await res.json();
      if (json?.error) {
        console.log(`  … faucet refused ${address}: ${String(json.error.message).slice(0, 80)}`);
        return false;
      }
 ...
 * suite. PoolRisk's `_pay` posts its internal transfer with `on="finalized"`, deliberately:
